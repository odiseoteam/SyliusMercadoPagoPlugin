...
Feature: Adding a new payment method
    ...
    @ui
    Scenario: Adding a new mercado pago payment method
        Given I want to create a new payment method with "Mercado Pago" gateway factory
        When I name it "Mercado Pago" in "English (United States)"
        And I specify its code as "MP"
        And I configure it with test mercado pago gateway data
        And I add it
        Then I should be notified that it has been successfully created
        And the payment method "Mercado Pago" should appear in the registry
