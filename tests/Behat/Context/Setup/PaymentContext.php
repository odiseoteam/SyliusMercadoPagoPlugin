...
namespace Tests\Odiseo\SyliusMercadoPagoPlugin\Behat\Context\Setup;
...
final class PaymentContext implements Context
{
    ...
    /**
     * @Given the store has (also) a payment method :paymentMethodName with a code :paymentMethodCode and Mercado Pago gateway
     */
    public function theStoreHasPaymentMethodWithCodeAndMercadoPagoGateway(
        $paymentMethodName,
        $paymentMethodCode
    ) {
        $paymentMethod = $this->createPaymentMethod($paymentMethodName, $paymentMethodCode, 'Mercado Pago');
        $paymentMethod->getGatewayConfig()->setConfig([
            'access_token' => 'TEST',
            'payum.http_client' => '@sylius.payum.http_client',
            'sandbox' => true,
